/**
 * Video S3 Upload Service
 * Handles uploading video chunks to S3 using presigned POST URLs
 */
...
export interface VideoS3Config {
    presignedPost: PresignedPost;
}

export class VideoS3UploadService {
    private readonly config: VideoS3Config;

    constructor(config: VideoS3Config) {
        this.config = config;
    }
    ...
    /**
     * Upload video chunk to S3 using presigned POST
     */
    async uploadChunk(blob: Blob, metadata: VideoChunkMetadata): Promise<string> {
        ...
        try {
            ...
            console.log(`✅ Video chunk uploaded successfully: ${filename}`);
            return url;
